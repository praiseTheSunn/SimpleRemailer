...
class MixNode:
    def __init__(self, asymmetric_encrytion_manager, symmetric_encryption_manager, send_strategy):
    ...
    def encrypt_with_key(self, algorithm_name: str, public_key, data: bytes) -> bytes:
        ...
        encrypted_chunks = [self.asymmetric_encrytion_manager.encrypt_w_key(algorithm_name, public_key, chunk) for chunk in chunks]
    ...
    def decrypt_with_key(self, algorithm_name: str, private_key, encrypted_data: bytes) -> bytes:
        ...
        decrypted_chunks = [self.asymmetric_encrytion_manager.decrypt_w_key(algorithm_name, private_key, chunk) for chunk in chunks]
    ...
    def symmetric_multi_layer_encrypt(self, symmetric_keys: list, data: list, flags_begin: list, encrypted_content: bytes) -> bytes:
        # encrypted_data = data
        # encrypted_data = symmetric_keys[0]
        # emailBytes = json.dumps(email.model_dump()).encode('utf-8')
        content = Hidden(ip=base64.b64encode(data[0]).decode('utf-8'), path_strategy="", flag_begin=flags_begin[0], flag_end=False, content=base64.b64encode(encrypted_content).decode('utf-8'))
        ...
        for i in range(0, len(symmetric_keys)):
            ...
            encrypted_data = self.symmetric_encryption_manager.encrypt(self.symmetric_algorithm_name, symmetric_keys[i], encrypted_data)
            ...
            hidden = Hidden(ip=dataStr, path_strategy="", flag_begin=flags_begin[i], flag_end=False, content=contentCipherStr)
            # print(kencrypted)
    ...
    def decrypt_content(self, symmetric_key, encrypted_message: bytes):
        decrypted_data = self.symmetric_encryption_manager.decrypt(self.symmetric_algorithm_name, symmetric_key, encrypted_message)
    ...
    def receive_and_add_to_queue(self, encrypted_message: Message):
        ...
        decrypted_symmetric_key, next_encrypted_key = self.decrypt_symmetric_key(encryption_algorithm, encrypted_key_bytes)
        # print("\nDECRYPTED SYM KEY: \n")
        ...
        # print(len(encrypted_content_bytes[16:]))
        next_ip, flag_begin, next_encrypted_content = self.decrypt_content(decrypted_symmetric_key, encrypted_content_bytes)
        # print("\nDECRYPTED CONTENT: \n")
        ...
        if (flag_begin):
            ...
            next_encrypted_content = self.symmetric_multi_layer_encrypt(symmetric_keys, paths_bytes, flags, next_encrypted_content)
            print("DONE ENCRYPTED CONTENT")
            next_encrypted_key = self.asymmetric_multi_layer_encrypt(new_path_encryption_algorithm, public_keys, symmetric_keys)
